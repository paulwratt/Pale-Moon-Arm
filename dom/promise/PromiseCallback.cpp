...
WrapperPromiseCallback::Call(const Optional<JS::Handle<JS::Value> >& aValue)
{
  ...
  // If invoking callback threw an exception, run promise's reject with the
  // thrown exception as an argument and the synchronous flag set.
  ...
  // Otherwise, run promise's resolve with value and the synchronous flag
  // set.
                         ...
                         Task aTask)
{
  ...
  // If we have a callback and a next promise, we have to exec the callback and
  // then propagate the return value to the next promise->resolve().
  if (aCallback) {
  ...
  MOZ_ASSERT(false, "PromiseCallback Factory: unknown task and no callback -- this should not happen");
  return nullptr;
}
